...
import { Eye, EyeOff, ArrowLeft, Camera, Upload, X, Image, Users } from "lucide-react";
...
import DesktopRegister from "@/components/desktop-register";
import { LogoIcon } from "@/components/logo";

export default function Register() {
  const isMobile = useIsMobile();

  // Se for desktop, usa o componente desktop
  if (!isMobile) {
  ...
  // Scroll para o topo e evitar auto-preenchimento do "Lembrar-me" do login
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center p-4 relative">
      {/* Background decorativo similar ao desktop */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute animate-float-slow top-16 left-16 w-8 h-8 opacity-15">
          <div className="w-full h-1 bg-gradient-to-r from-blue-400 to-blue-600 rounded-full absolute top-1/2 transform -translate-y-1/2"></div>
          <div className="w-1 h-full bg-gradient-to-b from-blue-400 to-blue-600 rounded-full absolute left-1/2 transform -translate-x-1/2"></div>
        </div>
        <div className="absolute animate-float-medium bottom-20 right-16 w-6 h-6 opacity-20">
          <div className="w-full h-full bg-gradient-to-br from-green-400 to-teal-400 rounded-full"></div>
        </div>
        <div className="absolute animate-float-reverse top-1/3 right-8 w-4 h-4 opacity-25">
      ...
      <Card className="w-full max-w-md border-0 shadow-xl bg-white/80 backdrop-blur-sm">
        <CardContent className="space-y-4 px-6 py-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-gray-600 text-lg">Cadastre-se no Meu Cuidador</p>
              </div>
            </div>
          ...
          <div className="relative w-full mb-4">
            <div className="absolute left-0 right-0 h-px bg-gray-200 -ml-6 -mr-6"></div>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Campo de upload de foto */}
            <div className="space-y-2">
              <div className="flex flex-col items-center space-y-3 pt-4 pb-4">
                {photoPreview ? (
                ...
                <Button
                  ...
                  className={`text-sm ${errors.photo ? "border-red-500 text-red-600 hover:bg-red-50" : ""}`}
                >
                  <Camera className="w-4 h-4 mr-2" />
                  {photoPreview ? "Alterar" : "Adicionar"} Foto
                </Button>
                {errors.photo && (
                  <p className="text-red-500 text-sm text-center">{errors.photo}</p>
                )}
              </div>
              {/* Input para galeria */}
              <input
            ...
            <div className="space-y-2">
              ...
              {formData.password && (
                <div className="space-y-2">
                  ...
                  <div className={`text-sm ${formData.password.length >= 8 && /[A-Z]/.test(formData.password) && /[a-z]/.test(formData.password) && /\d/.test(formData.password) && /[!@#$%^&*(),.?":{}|<>]/.test(formData.password) ? 'text-green-600' : 'text-gray-600'}`}>
            ...
            <Button
              type="submit"
              className="w-full h-11 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold transition-all duration-200 text-base md:text-sm mt-4"
              disabled={loading}
            >
